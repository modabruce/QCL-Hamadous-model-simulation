...
clear
clc
%folder = 'C:\Users\bruce\Desktop\QCL figures\steady_values';  % folder saving images

% r: ratio of J/Jth
r_values = linspace(1.1, 1.5, 5);  % reproduce results in LaserMatrix
tau_21_values = 2.0e-12;
...
%% save figures
% %folder = 'C:\Users\bruce\Desktop\QCL figures\steady_values';
% filename_J = 'J_varying.png';
...
for i = 1:length(tau_21_values)
    N3_test(1,i) = 48*N_steady_Hamadous{1,r}(1);  % 48 times fitted MAT to LM
    Nph_test(1,i) = N_steady_Hamadous{1,r}(4);
    SL_Q_test(1,i) = N_steady_SL{1,r}(1);
    SL_S_test(1,i) = N_steady_SL{1,r}(2);
end
...
plot(r_values, omega1_hamadous, 'o-', 'MarkerFaceColor', 'blue', 'DisplayName', 'Hamadous'); % Circle markers
hold on
plot(r_values * 1e12, omega1_SL, 's--', 'MarkerFaceColor', 'red', 'DisplayName', 'SL'); % Square markers
xlabel('r values');
ylabel('First pole frequency (Ghz)');
title('\omega_{1} for Different r Values');
legend show; 
grid on;
hold off;
...
title('Zero point for Different r Values');
legend show; 
grid on;
hold off;
...
for i = 1:length(r_values)
    ...
    plot(abs(freq) / 1e9, 20 * log10(abs(photon_response_hamadous)), 'DisplayName', ['r = ', num2str(r)],'LineWidth', 2);
end
...
for i = 1:length(r_values)
    ...
    % Plot the photon response against the frequency (in Hz)
    plot(abs(freq) / 1e9, 20 * log10(abs(photon_response_SL)), 'DisplayName', ['r = ', num2str(r)],'LineWidth', 2);
end
...
for i = 1:length(r_values)
    ...
    plot(abs(freq) / 1e9, ...
         ...
         'Color', line_color, ...
         'DisplayName', ['Hamadous: r= ', num2str(r), ' ps']);
...
log_mean_squared_difference = zeros(1,length(r_values));
lin_mean_squared_difference = zeros(1,length(r_values));
log_meandB = cell(1,length(r_values));
lin_meandB = cell(1,length(r_values));
 
for i = 1:length(r_values)
    r = r_values(i);

    % Extract the number_of_photons for the current tau_21 (for j=1 since tau_21 has only one value)
...
for i = 1:length(r_values)
    r = r_values(i);
% Plot the photon response against the frequency (in Hz)
plot(abs(freq)/1e9, log_meandB{j}, 'DisplayName', ['log error r = ', num2str(r), ' ps'],'LineWidth', 2);
% Plot the photon response againsjt the frequency (in Hz)
plot(abs(freq)/1e9, lin_meandB{j}, 'DisplayName', ['lin error r = ', num2str(r), ' ps'],'LineWidth', 2);
end
...
set(gca, 'XScale', 'log');
% in log devided by 2
plot(r_values, (log_mean_squared_difference), 'DisplayName', 'log mean squared difference','LineWidth', 2);
hold on; % Keep the plots on the same figure
% Add labels and legend
plot(r_values, (lin_mean_squared_difference), 'DisplayName', 'lin mean squared difference','LineWidth', 2);
...
for i = 1:length(r_values)
    ...
    zeros_freq_hm = zero_frequencies_hamadous{1,i};  % zero frequency
    poles_freq_hm = pole_frequencies_hamadous{1,i};  % all poles frequencies
    poles_freq_SL = pole_frequencies_SL{1,i};

    zero_point_hamadous = AM_response_solver_hamadous(abs(zeros_freq_hm), tau_21, N_steady_Hamadous{1,i});
    pole_point_hamadous = AM_response_solver_hamadous(abs(poles_freq_hm), tau_21, N_steady_Hamadous{1,i});

    pole_point_SL = 48*AM_response_solver_SL(abs(poles_freq_SL), tau_21, N_steady_SL{1,i});
   ...
   % Plot photon response with solid line
    plot(abs(freq) / 1e9, ...
         ...
         'Color', line_color, ...
         'LineStyle', '-', ...
         'DisplayName', ['Hamadous Response: r = ', num2str(r)]);
     ...
     plot(abs(freq) / 1e9, ...
         ...
         'Color', line_color, ...
         'LineStyle', '--', ...
         'DisplayName', ['SL Response: r = ', num2str(r)]);
    ...
    plot(abs(zeros_freq_hm) / 1e9, ...
         ...
         'DisplayName', ['hamadous zeros: r = ', num2str(r)]);
    ...
    plot(abs(poles_freq_hm) / 1e9, ...
         ...
         'LineWidth', 3, ...
         'Color', line_color, ...
         'DisplayName', ['hamadous poles: r = ', num2str(r)]);
    ...
    plot(abs(poles_freq_SL) / 1e9, ...
         ...
         'LineWidth', 3, ...
         'Color', line_color, ...
         'DisplayName', ['SL poles: r = ', num2str(r)]);
end
...
for i = 1:length(r_values)
    ...
    % Extract zero and pole frequencies for current tau_21
    zeros_freq_hm = zero_frequencies_hamadous{1,i};  % zero frequency
    poles_freq_hm = pole_frequencies_hamadous{1,i}(1);  % first roll off frequency
...
title('Photon Response', 'FontSize', 12);
legend('show', 'Location', 'southwest', 'FontSize', 12);
grid on;

%% test plotting log and lin error
% figure(11)
% % in log devided by 2
% plot(abs(freq)/1e9, (log_squared_diff_plot), 'DisplayName', 'log squared difference','LineWidth', 2);
% hold on; % Keep the plots on the same figure
% set(gca, 'XScale', 'log')
% xlabel('freq in Ghz','FontSize', 12);
% ylabel('dB','FontSize', 12);
% title('log','FontSize', 12);
% legend('show', 'Location', 'southwest', 'FontSize', 12);
% grid on;
% hold off;
% 
% figure(12)
% % Add labels and legend
% plot(abs(freq)/1e9, (lin_squared_diff_plot), 'DisplayName', 'lin squared difference','LineWidth', 2);
% set(gca, 'XScale', 'log')
% xlabel('freq in Ghz','FontSize', 12);
% ylabel('dB','FontSize', 12);
